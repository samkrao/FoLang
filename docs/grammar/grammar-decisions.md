...
| `DECISION-LEX-008` | Active | `u8`, `u`, `U`, and `L` are encoding prefixes only when immediately adjacent to a character or string quote. Otherwise they are ordinary identifiers. |
...
| `DECISION-LIT-003` | Active | Character and string literals use selected C++-compatible encoding prefixes, escapes, universal character names, and raw-string forms. Adjacent string literals form one string-literal sequence. |
