# FoLang Grammar and Semantic Decision Register — Revision 23

- Grammar: `folang-r21.ebnf`
- Grammar SHA-256: `61cbf7a28bcb15a1d04380fdd5e33336046bad058dad4cca0ddfaad6061d3672`
- Language reference basis: `language-ref(46).md`
- Language reference SHA-256: `5456f3310529bc3cc187d49701dc6707cd03e3545875e273ba2f98e8317fe42e`
- Status: decision-complete grammar and semantic register aligned with the current language reference
- Planned syntax policy: productions described as planned in `language-ref(46).md` remain in the complete grammar unless explicitly removed. Release-specific availability is handled by the parser/compiler conformance profile.
- Revision 23 finalizes the uniform operator-registration model. Built-in and pre-declared symbols are registered by the language; custom symbols are registered once inside the fixed source-only operator library. All implementations use ordinary `mode=overload`, custom symbols may have multiple distinct overload signatures, ASCII symbols obey ordinary maximal munch, and conceptual consistency is recommended rather than compiler-enforced.
...
cross ordinary library boundaries. The ordinary lexer uses maximal munch after
combining the language-owned registrations with the current compilation's local
custom declarations; for example, after `+-` is registered, `a+-b` uses `+-`,
while `a + -b` uses `+` followed by unary `-`.
...
| `DECISION-OPDECL-004` | Active | The operator source area is parsed by a separate grammar rooted at `operator-source-file`. The exact outer declaration is `@co.dap.library(type=operator) _ co.lang.library = { ... }`, and the body admits only operator registrations. `co.lang.operator` is absent from ordinary declaration kinds. Both the dedicated and ordinary lexers use maximal munch over their completed operator tables. Declaring an ASCII sequence intentionally changes adjacent tokenization; whitespace disambiguates shorter operators. Symbols beginning with `//` or `/*` are rejected because comment openers take lexical priority. |
