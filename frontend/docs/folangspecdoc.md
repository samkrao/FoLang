...
Difference betwee alias, new and opaque types

Alias is just another name for same type

x co.lang.type = co.lang.int;
y co.lang.type = co.lang.int;

x == y   //true
x == co.lang.int // true

New type is different type can hold same values 
x co.lang.newtype = co.lang.int;
y co.lang.newtype = co.lang.int;

x == y //false
x == co.lang.int //false
y == co.lang.int //false

opaque types are same as base type from which they derived but different from another opaque type

y co.lang.opaquetype = co.lang.int;
x co.lang.opaquetype = co.lang.int;
...
sub types are like new types but can be converted to base type without explicit type cast

// Supertype / contravariant
test co.lang.supertype = co.lang.int;

super types are like new types but base types can be converted to super type without explicit type cast
```
...
```
```
somefun(value co.lang.int)->(chooseType)={
    (value < 100).return("hello").otherwise.return(co.const.true);

}
```
//here the chooseType will be evaluated with the same parameter of function prior to function
//basically the compiler rewrites like this

if we call somefun with value 60
```
x co.lang.type = chooseType(value);
```
then
```
somefun(value co.lang.int)->(x)={

}
```
x is eitherr boolean or string based on value 
...
Is it not looking like function object ?

    Yes but not exactly function object. lets see function object syntax
    
    
    add (x co.lang.int, y co.lang.int)->(co.lang.int)= { this.return x+ y};
    somevar := add
    or 
    somevar := (x co.lang.int, y co.lang.int)->(co.lang.int)={ this.return x+ y;}
    
What lazy means ?

   When we say x:= add(1,2); the result is not computed visualize as it is just storing function object and its parameters

   If we don't use x any where in our program add will never be invoked.

   When we say co.out.println(x); at this point add is invoked only once with 1 and 2 and result stored in x, and any future references to x it will be static like normal x := add(1,2); once it invoke never invoke add function again.

Is it like a thunk ?

   Yes exactly where thunk needs to be declared up front like

   ```
   x co.lang.int->(^) = add(1,2);
   
   @co.dap.lazy
   x:=add(1,2);   is short cut
   ```
...
***Note:*** 
   For Functions and types ****the annotation @co.dap.extern**** is optional
...
Structurally they look almost identical — both are just lists of type and function contracts. The difference is **who implements them and how**.
...
| Origin | ML/OCaml modules | Java/C# interfaces |

---

They are not the same thing even if they look similar syntactically:

- A `signature` is a **structural contract** — it can include types, structs, nested definitions. It describes a whole capability unit.
- An `interface` is a **behavioral contract** — methods only, tied to OOP dispatch, virtual calls, polymorphism.


---
```
co.lang.signature  → for modules   (keep)
co.lang.interface   → for classes   (instead of interface)
```
---

### Structs, Modules and Classes
